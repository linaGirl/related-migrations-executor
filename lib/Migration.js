            ...
            fs.readdir(migrationPath, function (err, files) {
                if (err) callback(err);
                    ...
                    if (!files.length) callback(null, schemaDirs);
                        ...
                        Promise.all(files.map(this._getSchemaDirectories.bind(this)).map(function (file) {
            ...
            fs.readdir(versionPath, function (err, files) {
                if (err) {
                    ...
                    if (!filesLength) {
                        ...
                        Promise.all(files.map(this._getSqlFiles.bind(this)).map(function (file) {
